  ...
  return (
    <div className="animate-fade-in-up">
      <header className="mb-20">
        ...
        <span className="text-brass font-black tracking-[0.4em] uppercase text-[10px] mb-4 block">Asset Modification</span>
        <h1 className="text-5xl font-black text-white tracking-tighter uppercase leading-none">Edit <br/>Produk.</h1>
        <p className="text-white/30 text-[10px] mt-4 uppercase tracking-[0.2em] font-black italic">Modifikasi Data Aset pada Manifest — ID: BRG-00{id}</p>
      </header>
            ...
            {(preview || existingImage) && (
              <div className="mb-6 flex items-center gap-8">
                <div className="porthole w-40 h-40 shadow-2xl border-brass/50 border-4">
                  <img src={preview || existingImage} alt="Product" className="w-full h-full object-cover" />
                </div>
                <div className="space-y-2">
                  ...
                  {preview && (
                    <button type="button" onClick={() => { setImage(null); setPreview(null); }} className="text-[9px] font-black uppercase tracking-widest text-red-500/60 hover:text-red-500 transition-colors">
                      ✕ Hapus Perubahan
                    </button>
                  )}
                </div>
            ...
            <label className="block w-full bg-[#001f3f] border-2 border-dashed border-white/10 rounded-[3rem] px-12 py-12 text-center cursor-pointer hover:border-brass/30 transition-all duration-500 group">
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 group-hover:text-white/40 transition-colors">
                  {image ? image.name : 'Ganti Gambar — Pilih File Baru'}
                </span>
              </div>
            </label>
          </div>
